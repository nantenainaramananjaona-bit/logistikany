...
export default function App() {
  ...
  const [activeInputTab, setActiveInputTab] = useState<'meta' | 'strategy' | 'colors'>('colors');
  ...
  const handleExcelExport = async () => {
    try {
      ...
      await exportToExcel(activeResults, meta, sizesInputsMapping, printColumns);
    } catch (err: any) {
  ...
  return (
    <div className={`min-h-screen font-sans ${darkMode ? 'bg-[#0f1117] text-[#e8eaf0]' : 'bg-[#f0f2f8] text-[#1a1d2e]'} transition-colors duration-300 pb-12`}>
      ...
      {/* Header bar */}
      <header className={`max-w-7xl mx-auto px-4 py-4 md:py-6 flex flex-wrap items-center justify-between gap-4 border-b ${darkMode ? 'border-slate-800' : 'border-slate-200'} mb-6 print:hidden`}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              ...
              <p className="text-[9px] uppercase font-mono tracking-widest text-slate-500 font-semibold">LOGISTICS OFFICER ENVIRONMENT</p>
            </div>
          </div>
        </div>

        <div className="flex items-center gap-3 flex-wrap">
          <span className="text-[11px] font-mono bg-blue-500/10 border border-blue-500/20 text-[#4f8ef7] px-3 py-1 rounded-full tracking-wider font-bold">
            v10.0 — REACT ESP
          </span>
          ...
          <button
            ...
            className={`p-2 rounded-lg border cursor-pointer transition-all hover:scale-[1.02] active:scale-[0.98] ${
              darkMode ? 'border-slate-800 bg-slate-900 text-amber-405 hover:text-white' : 'border-slate-300 bg-white text-slate-605 hover:text-black'
            }`}
          >
      ...
      <main className="max-w-7xl mx-auto px-4 space-y-6 print:px-0 pb-16">

        {/* TOP COMPREHENSIVE ACTIONS BAR — ALIGNED RIGHT */}
        <div className="sticky top-2 z-50 print:hidden drop-shadow-md">
          <div className={`p-4 rounded-xl border flex flex-col md:flex-row items-center justify-between gap-4 transition-all duration-300 ${
            ...
            <div className="flex items-center gap-3 w-full md:w-auto">
              <span className="flex h-2.5 w-2.5 relative">
                {hasGenerated ? (
                  ...
                  <>
                    <span className="animate-pulse absolute inline-flex h-full w-full rounded-full bg-blue-450 opacity-75"></span>
                    <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-blue-400"></span>
                  </>
                )}
              </span>
              <div>
                <h2 className="text-xs font-mono font-extrabold uppercase tracking-widest text-[#4f8ef7]">
                  CONSOLE DE PILOTAGE
                </h2>
                <div className="text-[11px] font-mono font-bold mt-0.5">
                  {hasGenerated ? (
                    <span className="text-emerald-450">PACKING LIST DISPONIBLE</span>
                  ) : (
                    <span className={darkMode ? "text-slate-400" : "text-blue-200"}>EN ATTENTE DE COMPILATION</span>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 md:justify-end w-full md:w-auto">
              ...
              <button
                ...
                className="px-4 py-2 bg-gradient-to-r from-blue-550 via-[#4f8ef7] to-[#9b72f5] hover:brightness-110 text-white font-bold rounded-lg text-xs transition-all focus:outline-none flex items-center gap-1.5 cursor-pointer shadow-md shadow-blue-500/10 hover:scale-[1.01] active:scale-[0.99]"
              >
        ...
        {hasGenerated && (
          <div className="space-y-6 animate-fadeIn">
            ...
            {printSections.ind && results.map((res, ci) => {
              const bgClass = darkMode ? BG_COLORS_DARK[ci % PALETTE.length] : BG_COLORS_LIGHT[ci % PALETTE.length];
              const activeColorSizes = res.tailles.filter(t => isStandardSizeAlwaysShown(t) || (colors[ci]?.sizes[t]?.qtyTot || 0) > 0);
              return (
                <div
                  key={ci}
                  ...
                  <div className="overflow-x-auto rounded-lg border border-slate-855 bg-slate-900/10">
                    <table className="w-full text-xs text-center border-collapse">
                      ...
                      <tbody className="divide-y divide-slate-800/60 font-mono font-medium">
                        ...
                        {/* Totals row index */}
                        <tr className="bg-[#222636]/30 font-bold border-t border-slate-700 divide-x divide-slate-800">
                          {printColumns.ctn && <td colSpan={2} className="py-2.5 px-3 text-center col-ctn-index">TOTALE</td>}
                          {printColumns.color && <td className="px-2 text-slate-300 col-color-lbl">{res.nom}</td>}
                          {(() => {
                            const origColor = colors.find(c => c.nom === res.nom);
                            const showSkuCol = printColumns.sku && !!(origColor && Object.values(origColor.sizes || {}).some((s: any) => s.sku && String(s.sku).trim() !== ''));
                          ...
                          {printColumns.totalqty && <td className="px-2 text-slate-100 col-totalqty-metric">{res.totals.p}</td>}
                          {printColumns.net && <td className="px-2 text-teal-400 col-net-metric">{res.totals.n.toFixed(2)}</td>}
                          {printColumns.gross && <td className="px-2 text-red-400 col-gross-metric">{res.totals.g.toFixed(2)}</td>}
            ...
            {printSections.cpl && results.length > 1 && (
                    ...
                    <div className="overflow-x-auto rounded-lg border border-slate-800">
                      <table className="w-full text-xs text-center border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-800 font-mono font-medium">
                          ...
                          <tr className="bg-[#222636]/20 font-bold border-t border-slate-700 divide-x divide-slate-800">
                            <td className="py-2 px-3 text-left">TOTAL SHIFT</td>
                            {summaryUniqueSizes.map(t => {
                              let sumT = 0;
                              results.forEach(r => { sumT += r.totals.sizes[t] || 0; });
                              return <td key={t} className="font-extrabold">{sumT}</td>;
                            })}
                            <td className="font-extrabold text-slate-100">{grandTotals.p}</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
            ...
            {printSections.bk && results.length === 1 && (
                  ...
                  <table className="w-full text-xs text-center border-collapse">
                    <thead>
                      <tr className="bg-slate-900 font-mono font-semibold text-slate-400 border-b border-slate-800">
                        ...
                        {results[0].tailles.filter(t => isStandardSizeAlwaysShown(t) || (colors[0]?.sizes[t]?.qtyTot || 0) > 0).map(t => <th key={t} className="border-r border-slate-800">{t}</th>)}
                        <th>TOTAL QTY</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-800 font-mono font-medium">
                      <tr className="divide-x divide-slate-850">
                        <td className="py-2 px-3 text-left font-bold" style={{ color: results[0].color }}>{results[0].nom}</td>
                        {results[0].tailles.filter(t => isStandardSizeAlwaysShown(t) || (colors[0]?.sizes[t]?.qtyTot || 0) > 0).map(t => (
                          <td key={t} className="font-bold">{results[0].totals.sizes[t] || ''}</td>
                        ))}
                        <td className="font-black text-slate-100">{results[0].totals.p}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
                ...
                {printSections.stats && (
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4 print-stats-box break-inside-avoid">
                    <div className="p-3 rounded-lg border border-slate-800 bg-slate-900/10">
                      ...
                      <div className="text-sm font-bold font-mono text-white mt-0.5">{results[0].totals.c}</div>
                    </div>
                    <div className="p-3 rounded-lg border border-slate-800 bg-slate-900/10">
                      <div className="text-[9px] font-mono text-slate-500 font-bold uppercase">Poids Net :</div>
                      <div className="text-sm font-bold font-mono text-teal-450 mt-0.5">{results[0].totals.n.toFixed(2)} KG</div>
                    </div>
                    <div className="p-3 rounded-lg border border-slate-800 bg-slate-900/10">
                      <div className="text-[9px] font-mono text-slate-500 font-bold uppercase">Poids Brut :</div>
                      <div className="text-sm font-bold font-mono text-red-400 mt-0.5">{results[0].totals.g.toFixed(2)} KG</div>
                    </div>
                    <div className="p-3 rounded-lg border border-slate-800 bg-slate-900/10">
                      <div className="text-[9px] font-mono text-slate-500 font-bold uppercase">Volume Total :</div>
                      <div className="text-sm font-bold font-mono text-blue-450 mt-0.5">{results[0].totals.v.toFixed(4)} m³</div>
                    </div>
                  </div>
                )}
              </div>
